...
public class AccountApi {
	...
	public List<AccountDto> getAll(){
		return accountService.getAll().stream().map(Converter::convertToAccountDto).collect(Collectors.toList());
	}
	...
	public ResponseEntity<Account> addAccount(@RequestBody @Valid AccountDto accountDto){
		Account convertToAccount = Converter.convertToAccount(accountDto);
		return ResponseEntity.status(HttpStatus.CREATED).body(accountService.addAccount(convertToAccount));
	}
